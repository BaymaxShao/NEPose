...
from dataloader_NEPose import *
from dataloader_simcol import *
from dataloader_EndoSLAM import *
from loss import *
from utils import *
import numpy as np
import os
...
def main():
    args = Tester().parse()
    test_obj = [folder.split('/')[1][:-1] for folder in open(args.data+'/test_file.txt')][0]
    weights = torch.load(args.pretrained_model1)
    model = models.PoseNet().to(device)
    model = torch.nn.DataParallel(model, device_ids=[0])
    model.load_state_dict(weights['state_dict'], strict=True)
    model.eval()
    print('=== Pretrained Model Loaded ===')

    if args.dataset == 'simcol':
        test_set = SeqData_simcol(
            args.data,
            istrain=False,
            img_size=224,
    ...
    elif args.dataset == 'nepose':
        test_set = SeqData_NEPose(
            args.data,
            istrain=False,
            img_size=224,
            train_list='/train_file.txt',
    ...
    ATEs, RTEs, ROTs, CEs, DEs, traj_pred, traj_gt, d_pred, d_gt, rel_pred, rel_gt, length = test(test_loader, model, args.opt)
    # ATEs, RTEs, ROTs, CEs, DEs, traj_pred, traj_gt, d_pred, d_gt, rel_pred, rel_gt = test_pair(test_loader, model)
    ...
    print('The average CE of the trajectory: {:.4f}'.format(np.mean(CEs)*1000))
    ...
    print('The std CE of the trajectory: {:.4f}'.format(np.std(CEs)*1000))
...
def test(test_loader, model, opt):
    ...
    next_pose_pred = None
    length = 0
    for i, (img1, img2, flow, ref_pose, tar_pose, rel_pose) in enumerate(test_loader):
...
def test_rel(test_loader, model):
    ...
    for i, (img1, img2, flow, ref_pose, tar_pose, rel_pose) in enumerate(test_loader):
        ...
        # Predict the next pose
        next_pose_pred = np.matmul(ref_pose_T, T_pred)
        length += cal_distance(traj_gt[-1], next_pose_gt)
    ...
    return np.array(ATEs), np.array(RTEs), np.array(ROTs), np.array(CEs), np.array(DEs), traj_pred, traj_gt, d_pred, d_gt, rel_poses_pred, rel_poses_gt


def save_results(save_path, obj, ates, rtes, rots, ces, des, traj_pred, traj_gt, d_pred, d_gt, rel_pred, rel_gt):
    os.makedirs(str(save_path) + '/results_{}'.format(obj), exist_ok=True)
    ...
    # f = open(str(args.save_path) + '/results_{}'.format(timestamp) + '/MSEs_traj.txt', 'w')
    # for e in MSEs_traj:
    #     f.write(str(e) + '\n')
    # f.close()
    # f = open(str(args.save_path) + '/results_{}'.format(timestamp) + '/ATEs.txt', 'w')
    # for e in ATEs:
    #     f.write(str(e) + '\n')
    # f.close()
    # f = open(str(args.save_path) + '/results_{}'.format(timestamp) + '/RTEs.txt', 'w')
    # for e in RTEs:
    #     f.write(str(e) + '\n')
    # f.close()
    # f = open(str(args.save_path) + '/results_{}'.format(timestamp) + '/ROTs.txt', 'w')
    # for e in ROTs:
    #     f.write(str(e) + '\n')
    # f.close()
    # f = open(str(args.save_path) + '/results_{}'.format(timestamp) + '/CEs.txt', 'w')
    # for e in CEs:
    #     f.write(str(e) + '\n')
    # f.close()
    # f = open(str(args.save_path) + '/results_{}'.format(timestamp) + '/DEs.txt', 'w')
    # for e in DEs:
    #     f.write(str(e) + '\n')
    # f.close()
    # f = open(str(args.save_path) + '/results_{}'.format(timestamp) + '/traj_pred.txt', 'w')
    # for pose in traj_pred:
    #     x = pose[0, -1]
    #     y = pose[1, -1]
    #     z = pose[2, -1]
    ...
    #     f.write(str(z) + '\n')
    # f.close()
    # f = open(str(args.save_path) + '/results_{}'.format(timestamp) + '/traj_gt.txt', 'w')
    # for pose in traj_gt:
    #     x = pose[0, -1]
    #     y = pose[1, -1]
    #     z = pose[2, -1]
    ...
    #     f.write(str(z) + '\n')
    # f.close()
    # f = open(str(args.save_path) + '/results_{}'.format(timestamp) + '/directions_pred.txt', 'w')
    # for d in d_pred:
    #     x = d[0]
    #     y = d[1]
    #     z = d[2]
    ...
    #     f.write(str(z) + '\n')
    # f.close()
    # f = open(str(args.save_path) + '/results_{}'.format(timestamp) + '/directions_gt.txt', 'w')
    # for d in d_gt:
    #     x = d[0]
    #     y = d[1]
    #     z = d[2]
    ...
    #     f.write(str(z) + '\n')
    # f.close()
    # f = open(str(args.save_path) + '/results_{}'.format(timestamp) + '/T_pred.txt', 'w')
    # for t in rel_pred:
    #     f.write(str(t) + '\n')
    # f.close()
    # f = open(str(args.save_path) + '/results_{}'.format(timestamp) + '/T_gt.txt', 'w')
    # for t in rel_gt:
    #     f.write(str(t) + '\n')
    # f.close()
