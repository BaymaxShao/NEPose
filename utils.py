...
def ce(pose_gt, pose_pred):
    ...
    ce = ((1-np.cos((euler_pred[0]-euler_gt[0])*np.pi/180))+(1-np.cos((euler_pred[1]-euler_gt[1])*np.pi/180))+(1-np.cos((euler_pred[2]-euler_gt[2])*np.pi/180))) / 3
...
def procrustes(X, Y):
    """
    Procrustes analysis for point cloud registration.

    X, Y: numpy arrays of shape (n, d), where n is the number of points and d is the dimensionality.
    Returns the transformation matrix and the registered Y.
    """
    # Center the data
    ...
    U, S, Vt = np.linalg.svd(np.dot(Y_centered.T, X_centered))
    R = np.dot(U, Vt)

    # Calculate the scaling factor and apply it
    # scale = np.trace(np.dot(X_centered.T, X_centered)) / np.trace(np.dot(Y_centered.T, Y_centered))
    t = X_centroid - np.dot(R, Y_centroid)

    # Construct the transformation matrix
    T = np.eye(X.shape[1] + 1)
    T[:X.shape[1], :X.shape[1]] = R
    # T[:X.shape[1], -1] = t

    # Apply the transformation to Y
    Y_registered = np.dot(np.hstack((Y, np.ones((Y.shape[0], 1)))), T.T)[:, :X.shape[1]]

    return T, Y_registered
